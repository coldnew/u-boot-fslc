...
static void setup_iomux_lcdif(void)
{
	MX6QDL_SET_PAD(PAD_DI0_DISP_CLK__IPU1_DI0_DISP_CLK, 0);
	MX6QDL_SET_PAD(PAD_DI0_PIN2__IPU1_DI0_PIN2, 0);		// HSync 
	MX6QDL_SET_PAD(PAD_DI0_PIN3__IPU1_DI0_PIN3, 0);		// VSync 
	MX6QDL_SET_PAD(PAD_DI0_PIN4__IPU1_DI0_PIN4, 0);		// Contrast 
	MX6QDL_SET_PAD(PAD_DI0_PIN15__IPU1_DI0_PIN15, 0);		// DISP0_DRDY 
	MX6QDL_SET_PAD(PAD_DISP0_DAT0__IPU1_DISP0_DAT_0, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT1__IPU1_DISP0_DAT_1, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT2__IPU1_DISP0_DAT_2, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT3__IPU1_DISP0_DAT_3, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT4__IPU1_DISP0_DAT_4, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT5__IPU1_DISP0_DAT_5, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT6__IPU1_DISP0_DAT_6, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT7__IPU1_DISP0_DAT_7, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT8__IPU1_DISP0_DAT_8, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT9__IPU1_DISP0_DAT_9, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT10__IPU1_DISP0_DAT_10, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT11__IPU1_DISP0_DAT_11, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT12__IPU1_DISP0_DAT_12, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT13__IPU1_DISP0_DAT_13, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT14__IPU1_DISP0_DAT_14, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT15__IPU1_DISP0_DAT_15, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT16__IPU1_DISP0_DAT_16, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT17__IPU1_DISP0_DAT_17, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT18__IPU1_DISP0_DAT_18, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT19__IPU1_DISP0_DAT_19, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT20__IPU1_DISP0_DAT_20, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT21__IPU1_DISP0_DAT_21, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT22__IPU1_DISP0_DAT_22, 0);
	MX6QDL_SET_PAD(PAD_DISP0_DAT23__IPU1_DISP0_DAT_23, 0);
}

static void setup_iomux_lvds(void)
{
        MX6QDL_SET_PAD(PAD_SD4_DAT0__GPIO_2_8, 0);
        MX6QDL_SET_PAD(PAD_SD4_DAT1__GPIO_2_9, 0);
        MX6QDL_SET_PAD(PAD_SD4_DAT2__GPIO_2_10, 0);
        MX6QDL_SET_PAD(PAD_SD4_DAT3__GPIO_2_11, 0);
        
        MX6QDL_SET_PAD(PAD_LVDS0_CLK_P__LDB_LVDS0_CLK, 0);
        MX6QDL_SET_PAD(PAD_LVDS0_TX0_P__LDB_LVDS0_TX0, 0);
        MX6QDL_SET_PAD(PAD_LVDS0_TX1_P__LDB_LVDS0_TX1, 0);
        MX6QDL_SET_PAD(PAD_LVDS0_TX2_P__LDB_LVDS0_TX2, 0);
        MX6QDL_SET_PAD(PAD_LVDS0_TX3_P__LDB_LVDS0_TX3, 0);
}
...
static void enable_lcdif(void)
{
	setup_iomux_lcdif();
}

enum {
	FBMODE_PIXELCLOCK,       /* Pixel clock, in Hz */

	FBMODE_HRES,             /* Horizontal resolution, pixels */
	FBMODE_HFP,              /* Horizontal front porch */
	FBMODE_HBP,              /* Horizontal back porch */
	FBMODE_HSW,              /* Horizontal what? */
	...
	FBMODE_DATALINES,        /* Number of data lines (typically 24, but 18 exists too) */
	FBMODE_BPP,              /* Bits per pixel to use (unused parameter for now) */

	FBMODE_NOF_DATAFIELDS    /* Must be last in enum! */
};

static int panel_data[FBMODE_NOF_DATAFIELDS] =
        { 33260, 800, 210, 46, 1, 480, 22, 23, 1,  24, 32 };    /* Overwritten */

static struct fb_videomode custom_panel = {
	.name           = "Custom_Panel",
	.refresh        = 60,
	.xres           = 1024,
	.yres           = 600,
...
static struct fb_videomode const xga_1024_768 = {
	.name           = "xga_1024_768",
	.refresh        = 60,
	.xres           = 1024,
	.yres           = 768,
...
static struct fb_videomode const wsvga_1024_600 = {
	.name           = "wsvga_1024_600",
	.refresh        = 60,
	.xres           = 1024,
	.yres           = 600,
...
static struct fb_videomode const wvga_800_480 = {
	.name           = "wvga_800_480",
	.refresh        = 60,
	.xres           = 800,
	.yres           = 480,
...
static int envstr_to_disp_timing(char *inputs)
{
	int pclk=-1,xres=-1,hfp=-1,hbp=-1,hsw=-1,yres=-1,vfp=-1,vbp=-1,vsw=-1,bpp=-1,dataline=-1;
	int ret = 0;
	char * ep;

	pclk = simple_strtoul(inputs, &ep, 10);
	if(*ep)
		xres = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		hfp = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		hbp = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		hsw = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		yres = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		vfp = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		vbp = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		vsw = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		dataline = simple_strtoul(++ep, &ep, 10);
	if(*ep)
		bpp = simple_strtoul(++ep, &ep, 10);

	printf("Timing info :\n");
	printf("PCLK %d, XRES %d, YRES, %d\n",pclk,xres,yres);
	printf("Horizontal : HFP %d, HBP %d, HSW %d\n",hfp,hbp,hsw);
	printf("Vertical : VFP %d, VBP %d, VSW %d\n",vfp, vbp,vsw);
	printf("DATALINE %d ,BPP %d\n",dataline, bpp);

	custom_panel.refresh        = 60;
	custom_panel.xres           = 1024;
	custom_panel.yres           = 600;
	custom_panel.pixclock       = 15385;
	custom_panel.left_margin    = 220;
	custom_panel.right_margin   = 40;
	custom_panel.upper_margin   = 21;
	custom_panel.lower_margin   = 7;
	custom_panel.hsync_len      = 60;
	custom_panel.vsync_len      = 10;
	custom_panel.sync           = FB_SYNC_EXT;
	custom_panel.vmode          = FB_VMODE_NONINTERLACED;
}
...
#else
#define DISPLAY_TIMING	wsvga_1024_600
int board_video_skip(void)
{
	int ret;
	char *timing;

	timing = getenv("display_timing");
	if (timing != NULL) {
		envstr_to_disp_timing(timing);
	}

	ret = ipuv3_fb_init(&DISPLAY_TIMING, 0, IPU_PIX_FMT_RGB24);
...
static int splash_screen_read_file(block_dev_desc_t *dev, char *filename, ulong addr)
{
	int err = 0;
	err = fat_register_device(dev, CONFIG_SYS_MMC_SD_FAT_BOOT_PARTITION);
	if (err == 0) {
		err = file_fat_read(filename, (ulong *)addr, 0);
		if (err > 0) {
			printf("splash: loading %s from MMC FAT...\n", filename);
			return 0;
		} else {
			/* printf("splash:error reading %s from MMC FAT\n", filename); */
			/* no logo found in mmc fat */
		}
	} else {
		disk_partition_t info;
		err = get_partition_info(dev,
				CONFIG_SYS_MMC_SD_FAT_BOOT_PARTITION, &info);
		if (err == 0)
			err = ext4fs_probe(dev, &info);
		if (err == 0) {
			err = ext4_read_file(filename, (ulong *)addr, 0, 0);
			if (err > 0) {
				printf("splash: loading %s from MMC EXT...\n", filename);
				return 0;
			} else {
				/* printf("Error loading MMC EXT ...%s...\n", filename); */
				/* no logo found in mmc ext */
			}
		}
	}
	return -1;
...
int splash_screen_prepare(void)
{
	...
	if (s != NULL) {
		if ((s = getenv("splashimage_init_block")) != NULL)
			offset = simple_strtoul (s, NULL, 16);
		if ((s = getenv("splashimage_blkcnt")) != NULL)
		...
		case MX6_MMC_BOOT:
			...
			if (!mmc) {
				printf ("splash: Error Init MMC.\n");
				return 1;
			}
			mmc_init(mmc);
			if ((s = getenv("splashimage_file_name")) != NULL) {
				err = splash_screen_read_file(&mmc->block_dev, s, addr);
				if (err == 0)
					return 0;
			}
			if ((offset != 0) && (size != 0)) {
				...
				if (!offset)
					printf ("splashimage_init_block is illegal! \n");
				if (!size)
					printf ("splashimage_blkcnt is illegal! \n");
			}
			break;
		case MX6_SATA_BOOT:
			if (sata_curr_device == -1) {
				if (sata_initialize()) {
					printf ("splash:Error Init SATA.\n");
					return 1;
				}
				sata_curr_device = 0;
			}
			if((s = getenv("splashimage_file_name")) != NULL) {
				err = splash_screen_read_file(&sata_dev_desc[sata_curr_device], s, addr);
				if (err == 0)
					return 0;
			}
			if ((offset != 0) && (size != 0)) {
